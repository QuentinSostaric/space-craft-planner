...
import { SharedOfferOwnerIdentity } from "../organizations/SharedOfferOwner";
import {
...
export function MarketplaceBrowse({ model }: { model: MarketplaceController }) {
  ...
  const assets = useMemo(
    () =>
      route.type === "blueprints"
        ? activeDataset.blueprints.map((item) => ({
            label: item.name,
            value: item.id,
          }))
        : activeDataset.resources.map((item) => ({
            label: item.name,
            value: item.id,
          })),
  ...
  const blueprintOffers = members.flatMap((member) =>
    member.sharedBlueprintIds.flatMap((id) => {
      ...
      return blueprint && (!route.assetId || route.assetId === id)
        ? [{ member, blueprint }]
        : [];
    }),
  );
  ...
  const hiddenBlueprints = members.reduce(
    (sum, member) =>
      sum +
      member.sharedBlueprintIds.filter((id) => !blueprintById.has(id)).length,
    0,
  );
  const count =
    route.type === "contributors"
      ? members.length
      : route.type === "blueprints"
        ? blueprintOffers.length
  ...
  return (
    <Stack spacing={2.5}>
            ...
            {route.type !== "contributors" && (
              <AppSelect
                ...
                options={[
                  {
                    value: "",
                    label: t("All items", "Tous les éléments", "Alle Einträge"),
                  },
                  ...assets,
                ]}
                filterable
      ...
      >
        <Typography variant="body2" role="status">
          ...
          {route.type === "contributors"
            ? t(
                "contributors loaded",
                "contributeurs chargés",
                "Beitragende geladen",
              )
            : t("offers loaded", "offres chargées", "Angebote geladen")}{" "}
      ...
      {model.loading && !members.length && (
        <SurfaceState
          ...
          title={t(
            ...
            "Community-Angebote werden geladen",
          )}
        />
      )}
      ...
      >
        {route.type === "contributors"
          ? members.map((member) => (
              <Paper role="listitem" key={member.handle} sx={{ p: 2 }}>
                <SharedOfferOwnerIdentity
                ...
                >
                  {member.sharedBlueprintIds.length} blueprints ·{" "}
                  {member.sharedResources.length}{" "}
                  {t("resource lots", "lots de ressources", "Ressourcenlose")}
                </Typography>
                <Stack
                  direction="row"
                  spacing={1}
          ...
          : route.type === "blueprints"
            ? blueprintOffers.map(({ member, blueprint }) => {
                const activeRequest =
                  model.auth.account?.outgoingCraftRequests.find(
                    (request) =>
                      request.source === "community" &&
                      request.blueprintId === blueprint.id &&
                      request.ownerRsiHandle?.toLowerCase() ===
                ...
                return (
                  ...
                  >
                    <SharedBlueprintOfferCard
                      ...
                      onViewRequests={() =>
                        navigateToPath("/account?section=requests")
                      }
                  ...
                  <SharedResourceOfferCard
                    ...
                    onOpenResource={
                      resourceById.has(entry.resourceId)
                        ? () =>
                            navigateToPath(
                              resourcePathFromSlug(entry.resourceId),
                            )
                        : undefined
                    }
                    onContact={
