...
import type { Blueprint, ItemStats, GppModifier } from '../../types';
...
/* ─── Unique modifier dedup ─── */

function deduplicateModifiers(blueprint: Blueprint): GppModifier[] {
...
export function StatsSection({ blueprint, projectedStats, qualityScore, sectionRef }: StatsSectionProps) {
  ...
  const uniqueModifiers = deduplicateModifiers(blueprint);
  ...
  return (
    <Box ref={sectionRef} component="section" aria-label={t('Stats', 'Stats')} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
