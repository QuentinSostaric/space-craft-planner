...
export function SharedResourceOfferCard({ entry, resource, owner, contextLabel, onOpenResource, onContact, contactLabel, extraAction }: SharedResourceOfferCardProps) {
  ...
  return <Paper component="article" aria-label={`${entry.resourceName} · ${owner.displayName || owner.handle}`} sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5, height: '100%' }}>
    ...
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 'auto' }}>
      {onContact ? <AppButton variant="secondary" onClick={onContact} sx={{ minHeight: 44 }}>{contactLabel ?? t('View RSI profile', 'Voir le profil RSI', 'RSI-Profil ansehen')}</AppButton>
