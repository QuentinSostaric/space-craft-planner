...
export function AccountInventoryPanel({ model }: { model: AccountController }) {
  ...
  const filtered =
    ...
    sharingFilter !== "all";
  const openBlueprint = (
    entry: Extract<AccountLibraryEntry, { kind: "blueprint" }>,
    ...
    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
      ...
      <AppButton
        ...
        disabled={
          ...
          model.sharedResourceBusyId !== null
        }
      ...
      <Box sx={{ flex: 1, minWidth: 170 }}>
        <AppButton
          ...
          onClick={() =>
            entry.kind === "blueprint"
              ...
              : entry.resource &&
                navigateToPath(
                  resourcePathFromSlug(entry.resourceEntry.resourceId),
                  {
                    mainView: "resources",
                  },
                )
        ...
        >
          {entry.kind === "blueprint"
            ...
            : entry.resourceEntry.resourceName}
        </AppButton>
        <Typography
          variant="caption"
      ...
      {entry.kind === "resource" ? (
        ...
        <Box sx={{ display: "flex", gap: 0.5, flexWrap: "wrap" }}>
          <AppButton
            ...
            onClick={() =>
              model.handleToggleFavoriteBlueprint(entry.blueprint.id)
            }
          >
          ...
          <AppButton
            size="sm"
            variant="ghost"
            ariaPressed={entry.isInInventory}
            onClick={() =>
              model.handleToggleInventoryBlueprint(entry.blueprint.id)
            }
          >
          ...
          <AppButton
            size="sm"
            variant="ghost"
            onClick={() => openBlueprint(entry)}
          >
            {t("Simulate", "Simuler", "Simulieren")}
          </AppButton>
          <AppButton
            size="sm"
            variant="ghost"
  ...
  return (
    ...
    >
          ...
          >
            ...
            <AppSelect
              ...
              options={[
                {
                  value: "all",
                  label: t("All assets", "Tous les éléments", "Alle Einträge"),
                },
                {
                ...
                {
                  value: "favorite-blueprints",
                  label: t("Favorites", "Favoris", "Favoriten"),
                },
                {
                  value: "resources",
                  label: t("Resources", "Ressources", "Ressourcen"),
                },
              ]}
            />
          </Box>
          ...
          >
            <AppSelect
              ...
              onValueChange={(sharing) =>
                sharing && updatePreferences({ sharing })
              }
            ...
            <AppSelect
              ...
              options={[
                {
                  value: "name-asc",
                  label: t("Name A–Z", "Nom A–Z", "Name A–Z"),
                },
                {
                  value: "name-desc",
                  label: t("Name Z–A", "Nom Z–A", "Name Z–A"),
                },
                {
                  value: "recent",
                  label: t(
      ...
      >
        ...
        <AppButton
          ...
          disabled={
            !canManageOrganizations ||
            !model.linkedOrganizations.length ||
            !inventoryResources.length
          }
          onClick={model.openResourceBulkShareDialog}
        >
          {t(
            "Share resource batches",
            "Partager des lots de ressources",
            "Ressourcenbestände teilen",
          )}
        </AppButton>
      </Box>
      ...
      {(model.blueprintCollectionError ||
        model.sharedBlueprintError ||
        model.resourceCollectionError) && (
      ...
      {!filteredAssetEntries.length ? (
        <Paper
          variant="outlined"
          sx={{ px: 2, py: 5, textAlign: "center", borderStyle: "dashed" }}
        >
          ...
          <Typography
            sx={{ color: "text.secondary", mt: 1, mb: 2, fontSize: ".875rem" }}
          >
            {filtered
              ? t(
          ...
          <AppButton
            variant="secondary"
            onClick={filtered ? resetFilters : openPicker}
          >
            {filtered
              ? t(
                  "Reset filters",
          ...
          {visibleAssetEntries.map((entry, index) =>
            assetView === "list" ? (
              ...
              <BlueprintCard
                ...
                onSelect={(blueprint) =>
                  startTransition(() => model.setActiveBlueprint(blueprint))
                }
                onToggleFavorite={model.handleToggleFavoriteBlueprint}
                actionsAlwaysVisible
                onToggleInventory={model.handleToggleInventoryBlueprint}
              />
            ) : (
              <AccountResourceCard
                key={entry.key}
                entry={entry}
                actions={resourceActions(entry)}
              />
            ),
          )}
        </Box>
      ...
      {visibleAssetEntries.length < filteredAssetEntries.length && (
        <AppButton
          variant="secondary"
          onClick={() =>
            model.setVisibleBlueprintCount(
              (count) => count + ACCOUNT_BLUEPRINT_BATCH_SIZE,
            )
          }
        >
          {t("Load more", "Afficher plus", "Mehr anzeigen")} (
          {visibleAssetEntries.length}/{filteredAssetEntries.length})
        </AppButton>
      )}
      <AppDialog
        ...
        footer={
          <Box sx={{ display: "flex", gap: 1, justifyContent: "flex-end" }}>
            ...
            >
              {t("Add selected", "Ajouter la sélection", "Auswahl hinzufügen")}{" "}
                    ...
                    label={
                      <Box>
                        ...
                        >
                          {blueprint.category}
                          {owned
                            ? ` · ${t("Already owned", "Déjà possédé", "Bereits vorhanden")}`
                            : ""}
                        </Typography>
                      </Box>
                    }
                    onCheckedChange={(checked) =>
                      setSelectedBlueprints((current) =>
                        checked
                          ...
                          : current.filter((id) => id !== blueprint.id),
                      )
                    }
                  />
                </Box>
