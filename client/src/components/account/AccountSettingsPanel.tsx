...
function SettingsSection({
  ...
  return (
    <Paper
      component="section"
      variant="outlined"
      sx={{ p: { xs: 1.5, sm: 2 }, minWidth: 0, ...(fullWidth ? { gridColumn: '1 / -1' } : {}) }}
    >
      <Stack spacing={2}>
        <Box>
...
export function AccountSettingsPanel({ model }: { model: AccountController }) {
  ...
  const scopeLabel = activeDataset.channel === 'ptu' ? 'PTU' : 'LIVE';
  ...
  return (
    <Stack role="tabpanel" id="account-tabpanel-settings" aria-labelledby="account-tab-settings" spacing={2}>
      <Box>
      ...
      >
        ...
        >
          <Stack direction="row" spacing={1.25} alignItems="center">
            ...
            <Box sx={{ minWidth: 0, flex: 1 }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 700, overflowWrap: 'anywhere' }}>
                {account?.rsi?.handle ??
                  t('No RSI account linked', 'Aucun compte RSI lié', 'Kein RSI-Konto verknüpft')}
              </Typography>
              {profileUrl && (
                <Link href={profileUrl} target="_blank" rel="noopener noreferrer">
                  {t('View RSI profile', 'Voir le profil RSI', 'RSI-Profil ansehen')}
                </Link>
              )}
            </Box>
          </Stack>
          {account?.rsi?.handle && (
            <AppChip
              ...
              label={
                rsiVerificationRequired
                  ? t('Verification required', 'Vérification requise', 'Verifizierung erforderlich')
                  : t('Verified identity', 'Identité vérifiée', 'Verifizierte Identität')
              }
            />
          )}
          {(!account?.rsi?.handle || rsiVerificationRequired) &&
            (citizenIdRsiLinkEnabled ? (
              ...
              <Button variant="primary" onClick={handleStartRsiLink} disabled={model.rsiAction.busy}>
                {t('Verify RSI manually', 'Vérifier RSI manuellement', 'RSI manuell verifizieren')}
              </Button>
            ))}
          {account?.rsi?.handle && (
            <Button
            ...
            >
              {rsiUnlinkAction.busy
                ...
                : t('Unlink RSI account', 'Délier le compte RSI', 'RSI-Konto trennen')}
            </Button>
          )}
          {rsiUnlinkAction.error && <AppAlert severity="error">{rsiUnlinkAction.error}</AppAlert>}
        </SettingsSection>

        <SettingsSection
          fullWidth
          title={t('Data and backups', 'Données et sauvegardes', 'Daten und Sicherungen')}
          description={t(
            `You are managing your ${scopeLabel} account data. LIVE and PTU inventories are separate.`,
            `Tu gères les données de ton compte ${scopeLabel}. Les inventaires LIVE et PTU sont séparés.`,
            `Du verwaltest deine ${scopeLabel}-Kontodaten. LIVE- und PTU-Inventare sind getrennt.`,
          )}
        >
          <Stack
          ...
          >
            <Box>
              <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
                {t('Download a copy', 'Télécharger une copie', 'Kopie herunterladen')}
              </Typography>
              <Typography variant="body2" sx={{ mt: 0.25, color: 'text.secondary' }}>
                {t(
                  'Save your current profile, inventories, favorites, planner and shares as a JSON file.',
                  'Enregistre ton profil actuel, tes inventaires, favoris, planner et partages dans un fichier JSON.',
                  'Speichere dein aktuelles Profil, Inventare, Favoriten, deinen Planner und Freigaben als JSON-Datei.',
                )}
              </Typography>
            </Box>
            <Button variant="secondary" onClick={model.exportAccount} disabled={!account}>
              {t(
                `Export ${scopeLabel} JSON`,
                `Exporter le JSON ${scopeLabel}`,
                `${scopeLabel}-JSON exportieren`,
              )}
            </Button>
          </Stack>
          <Divider />
          ...
          >
            <Box>
              <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
                {t('Import from this device', 'Importer depuis cet appareil', 'Von diesem Gerät importieren')}
              </Typography>
              <Typography variant="body2" sx={{ mt: 0.25, color: 'text.secondary' }}>
            ...
            <Button
              variant="secondary"
              disabled={!model.localImportPlan.hasPendingImport || model.importAction.busy}
              onClick={() => model.setImportModalDismissed(false)}
            >
              {t('Review local import', 'Revoir l’import local', 'Lokalen Import prüfen')}
            </Button>
          </Stack>
          <Divider />
          <Box
          ...
          {copyLiveToPtuAction.error && <AppAlert severity="error">{copyLiveToPtuAction.error}</AppAlert>}
        </SettingsSection>
        ...
        {isDesktop && (
          ...
          >
            ...
            {customPaths.length > 0 && (
              <Box>
                <Typography component="h4" variant="subtitle2">
                  {t('Custom folders', 'Dossiers personnalisés', 'Eigene Ordner')}
                </Typography>
                <Stack spacing={1} sx={{ mt: 1 }}>
                  {customPaths.map((path) => (
                    <Stack key={path.id} direction="row" spacing={1} alignItems="center">
            ...
            >
              ...
              >
                <AppSelect
                  ...
                  options={['LIVE', 'PTU', 'HOTFIX', 'TECH-PREVIEW', 'EVOCATI'].map((label) => ({
                    value: label,
                    label,
                  }))}
                ...
                <AppTextField
                  ...
                  disabled={watcherBusy}
                />
                <Button type="submit" variant="secondary" disabled={!customPathInput.trim() || watcherBusy}>
            ...
            {(watcherError || watcher.error || sync.detectError) && (
              <AppAlert severity="error">{watcherError || watcher.error || sync.detectError}</AppAlert>
            )}
          </SettingsSection>
        )}
        ...
        >
          <Stack spacing={1.5}>
            <Typography component="h3" variant="h6" sx={{ color: 'error.main' }}>
            ...
            >
              ...
              >
                {deleteAction.busy
                  ...
                  : t('Delete my account', 'Supprimer mon compte', 'Mein Konto löschen')}
              </Button>
            </Stack>
            {deleteAction.error && <AppAlert severity="error">{deleteAction.error}</AppAlert>}
          </Stack>
        </Paper>
      </Box>
    </Stack>
