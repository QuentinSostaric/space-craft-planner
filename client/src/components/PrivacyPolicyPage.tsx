    ...
    <PageLayout width="reading" component="article">
      ...
      <Box>
        ...
        <Section title={t('3. How we use your data', '3. Utilisation des données', '3. Verwendung der Daten')}>
          ...
          <P>
            {t(
              'Community marketplace publication is optional and separate from organization sharing. When you publish, verified app members can see your public RSI handle and name, your selected blueprints, and the quantities and qualities of selected resource lots. Your Discord identity, organization memberships and unselected inventory are not included in these listings. LIVE and PTU selections are separate. You can withdraw your listings from Marketplace → My listings.',
              'La publication dans la marketplace communautaire est facultative et séparée des partages d’organisation. Lorsque vous publiez, les membres vérifiés de l’app peuvent voir votre handle et votre nom RSI publics, vos blueprints sélectionnés, ainsi que les quantités et qualités des lots choisis. Votre identité Discord, vos organisations et votre inventaire non sélectionné ne figurent pas dans ces annonces. Les sélections LIVE et PTU sont séparées. Vous pouvez retirer vos annonces depuis Marketplace → Mes annonces.',
              'Veröffentlichungen auf dem Community-Marktplatz sind freiwillig und getrennt von Organisationsfreigaben. Verifizierte App-Mitglieder sehen dabei Ihren öffentlichen RSI-Handle und Namen, ausgewählte Blueprints sowie Menge und Qualität ausgewählter Ressourcen. Discord-Identität, Mitgliedschaften und nicht ausgewähltes Inventar werden nicht veröffentlicht. LIVE- und PTU-Auswahlen sind getrennt. Unter Marktplatz → Meine Angebote können Sie Angebote zurückziehen.',
            )}
          </P>
          <P>
            {t(
              'Craft requests store the participants, requested blueprint, optional comment, resource arrangement and status so both players can follow the request. Blocking preferences are private. Marketplace reports store the reporter, reported player, selected reason and moderation status; they are available only to app moderators, not the reported player. Deleting your account removes your identity from reports you submitted; anonymized reports can remain for moderation.',
              'Les demandes de craft enregistrent les participants, le blueprint demandé, le commentaire facultatif, l’accord sur les ressources et le statut pour permettre leur suivi par les deux joueurs. Les préférences de blocage sont privées. Les signalements de marketplace enregistrent l’auteur, le joueur signalé, le motif choisi et le statut de modération ; ils sont accessibles uniquement aux modérateurs de l’app, pas au joueur signalé. La suppression de votre compte efface votre identité des signalements déposés ; les signalements anonymisés peuvent être conservés pour la modération.',
              'Craft-Anfragen speichern Beteiligte, Blueprint, optionalen Kommentar, Ressourcenabsprache und Status, damit beide Spieler die Anfrage verfolgen können. Blockiereinstellungen sind privat. Marktplatz-Meldungen speichern meldenden und gemeldeten Spieler, ausgewählten Grund und Moderationsstatus; sie sind nur für App-Moderatoren sichtbar, nicht für den gemeldeten Spieler. Bei Kontolöschung wird Ihre Identität aus eingereichten Meldungen entfernt; anonymisierte Meldungen können für Moderation erhalten bleiben.',
            )}
          </P>
          <P>
            {t(
